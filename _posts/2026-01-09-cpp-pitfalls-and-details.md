...
reinterpret_cast暴力指针和数之间的转换：
暴力完成两个完全无关类型的指针之间或指针和数之间的互转，比如用char类型指针指向double值。它对原始对象的位模式提供较低层次上的重新解释（即reinterpret），完全复制二进制比特位到目标对象，转换后的值与原始对象无关但比特位一致，前后无精度损失。即：简单的完成比特位复制，按照不同类型的读法会变成不同的值，但是比特位是一致。
...
unique_ptr原理:
unique_ptr 是一个独占型的智能指针（不允许其他的智能指针共享其内部的指针）， unique_ptr 对其持有的堆内存具有唯一拥有权，也就是说引用计数永远是 1 。因此一个非空的 unique_ptr 总是拥有它所指向的资源（保证同⼀时间内只有⼀个智能指针可以指向该对象），如果其离开作⽤域时，其指向对象将被销毁。
...
### 30.extern C的作用
这是 C++ 和 C 在底层链接机制上最大的不同。
C 语言：`不支持函数重载`。
函数 void foo(int a) 在编译后的符号表中，名字通常就是 _foo。
因为它不需要区分参数类型，名字是唯一的。
C++ 语言：`支持函数重载`（Overloading）。
你可以写 void foo(int a) 和 void foo(double a)。
为了在底层区分这两个同名函数，C++ 编译器会将函数名、参数类型编码成一个独特的符号。这叫 `Name Mangling`。
例如：void foo(int a) 可能会被编译成 `_Z3fooi`（GCC 规则，3代表名字长度，i代表参数int）。
冲突点： 如果你在 C++ 中直接引用一个 C 语言写的库函数 foo，C++ 编译器会去寻找 `_Z3fooi`，但 C 语言库里只有 `_foo`。链接器找不到符号，就会报错：Undefined Reference。
extern "C" 的作用就是强行关掉 C++ 的这个 Name Mangling 机制，让它生成（或寻找）干净的 `_foo` 符号。
...
4、auto不能被声明为返回值，auto不能作为形参，auto不能被修饰为模板参数。  
